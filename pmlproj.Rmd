...
We select for the predictors that have an importance higher than 30, since we observed from the plot above a diminishing contribution beyond that point. Since we had used only a subsample (10% of the training data set) in order to determine the features to be used, we now fit our final model with 10-fold cross-validation using the remaining 90% of the training data (previously stored as *trainingFinal*).
```{r cull_gbm_vimp}
# Cull using relative importance higher than 30
trainingCulled <- trainingFinal[, v$importance[,1]>=30]
