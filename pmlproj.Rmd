...
```{r transform_data_drop_correlated}
library(caret)
...
```{r transform_data_subset_traintest}
...
We proceed to use a different boosting model in order to improve on our decision tree. Using **gbm** we can check variable importance relative to other predictors out of the box. 
```{r fit_gbm}
library(gbm); library(survival); library(splines)
library(plyr); library(parallel)
modGBM <- train(classe~., method="gbm", data=sub1, verbose=FALSE)
...
We select for the predictors that have an importance higher than 30, since we observed from the plot above a diminishing contribution beyond that point. Since we had used only a subsample (10% of the training data set) in order to determine the features to be used, we now fit our final model with 3-fold cross-validation (not too many to reduce chance of overfitting) using the remaining 90% of the training data (previously stored as *trainingFinal*).
```{r cull_gbm_vimp}
# Cull using relative importance higher than 30
trainingCulled <- trainingFinal[, v$importance[,1]>=25]
names(trainingCulled[,-dim(trainingCulled)])
# Fit our final model with 10-fold cross-validation
...
+ All files and full code used are available from my [Github Project Repository](https://github.com/slothdev/PMLproj-Repo) (https://github.com/slothdev/PMLproj-Repo)
