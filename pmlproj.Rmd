...
### Using a gradient boost model to check variable importance
...
We select for the predictors that have an importance higher than 30, since we observed from the plot above a diminishing contribution beyond that point. Since we had used only a subsample (10% of the training data set) in order to determine the features to be used, we now train on our final model using the remaining 90% of the training data (previously stored as *trainingFinal*.
...
# Fit our final model
modGBMculled <- train(classe~., method="gbm", data=trainingCulled, verbose=FALSE)
